...
app = FastAPI(title="RouteCore Drone Delivery API", version="2.0")
...
def generate_random_location(center_lat: float, center_lon: float, max_radius_km: float):
    ...
    """
    while True:
        r = random.uniform(2.0, max_radius_km)        # min 2km so it's not ON the depot
        theta = random.uniform(0, 2 * math.pi)
        # Convert polar offset to lat/lon delta
...
def _seed_orders():
    ...
    global orders
    weights = [2.0, 4.0, 6.0, 1.5, 3.5, 8.0]
...
def route_cost(route_order_ids: List[str], order_map: Dict[str, Order],
    ...
    for oid in route_order_ids:
        ...
        total_dist += d
        energy += d * o.weight            # energy ∝ distance × weight
...
def generate_neighbor(solution: Dict[str, List[str]], drone_ids: List[str]) -> Dict[str, List[str]]:
    ...
    if op < 0.5:
        # Move one order from a random non-empty drone to another
        ...
        # Swap two orders between two different drones
        ...
        # Reverse a sub-route within a single drone
...
def run_simulated_annealing(current_drones: List[Drone], current_orders: List[Order]):
    """Core SA loop — updates sim_status in place."""
    global sim_status
    ...
    T_min = 0.5
    cooling = 0.995
    total_iters = int(math.log(T_min / T) / math.log(cooling))
    iteration = 0
    cost_history = []
    ...
    while T > T_min:
        ...
        if iteration % 200 == 0:
            cost_history.append(round(current_cost_val, 3))
            sim_status.iteration = iteration
            sim_status.total_iterations = total_iters
            sim_status.current_cost = round(current_cost_val, 3)
            sim_status.best_cost = round(best_cost_val, 3)
            sim_status.temperature = round(T, 4)
            sim_status.progress = round(min(iteration / max(total_iters, 1) * 100, 99.9), 1)
    ...
    for did, oids in best_solution.items():
        ...
        # Enforce hard constraints: remove orders until the route is valid
        ...
        assignments[did] = current_oids
        route_meta = {
            "id": did,
            "name": drone_map[did].name,
            "altitude": drone_map[did].altitude,
            "deliveries": len(current_oids),
            "total_delivered_so_far": drone_map[did].deliveries_count + len(current_oids)
        }
        ...
        actual_total_cost += dist + energy * 0.1
    ...
    # Update drone batteries globally
    for d in drones:
        if d.id in drone_metrics:
            d.current_battery = drone_metrics[d.id]["remaining_battery"]
            mission_count = len(assignments.get(d.id, []))
            d.deliveries_count += mission_count
            d.current_load = mission_count
            d.status = "delivering" if mission_count > 0 else "idle"
    ...
    sim_status.best_cost = round(actual_total_cost, 3)
    sim_status.result = {
        "assignments": assignments,
        "routes": routes,
        "drone_metrics": drone_metrics,
        "cost_history": cost_history,
        "total_cost": round(actual_total_cost, 3),
        "total_distance": round(sum(v["distance"] for v in drone_metrics.values()), 2),
        "total_energy": round(sum(v["energy"] for v in drone_metrics.values()), 2),
    }
...
async def add_order(data: OrderCreate):
    if data.weight <= 0 or data.weight > 50:
        raise HTTPException(status_code=400, detail="Weight must be between 0 and 50 kg")
    lat, lon = generate_random_location(DEPOT_LAT, DEPOT_LON, MAX_RADIUS_KM)
    dist = round(haversine(DEPOT_LAT, DEPOT_LON, lat, lon), 2)
...
async def start_simulation(background_tasks: BackgroundTasks):
    ...
    for o in orders:
        if o.weight > max_drone_capacity:
            raise HTTPException(
                status_code=400, 
                detail=f"Order {o.id} ({o.weight}kg) is too heavy. Max drone payload is {max_drone_capacity}kg. The drone is not able to do this task."
            )
        if (o.distance_to_depot * 2) > max_drone_range:
            raise HTTPException(
                status_code=400, 
                detail=f"Order {o.id} is {o.distance_to_depot:.1f}km away (requires {o.distance_to_depot * 2:.1f}km round-trip). Max drone range is {max_drone_range}km. The drone is not able to do this task."
            )

    # Reset drone batteries
    for d in drones:
